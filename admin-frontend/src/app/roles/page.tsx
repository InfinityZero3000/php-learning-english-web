...
export default function RolesPage() {
  ...
  async function assign(event: React.FormEvent) {
    ...
    try {
      const created = await roleManagement.assign(Number(teacherId), Number(learnerId));
  ...
  async function remove(id: number) {
    try {
      await roleManagement.remove(id);
    ...
    <div className="space-y-7">
      ...
      {loading ? <p className="rounded-3xl bg-white p-10 text-center font-bold">Loading controls…</p> : <>
        ...
        <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
          <form onSubmit={assign} className="rounded-3xl bg-white p-6" style={{ border: '2px solid #bdc8d2', borderBottomWidth: 4 }}><h3 className="text-xl font-black">Assign teacher</h3><label className="mt-5 block font-bold">Teacher<select required value={teacherId} onChange={(event) => setTeacherId(event.target.value)} className="mt-2 w-full rounded-xl px-4 py-3" style={{ border: '2px solid #bdc8d2' }}><option value="">Choose teacher</option>{teachers.map((user) => <option key={user.id} value={user.id}>{user.name} · {user.email}</option>)}</select></label><label className="mt-4 block font-bold">Learner<select required value={learnerId} onChange={(event) => setLearnerId(event.target.value)} className="mt-2 w-full rounded-xl px-4 py-3" style={{ border: '2px solid #bdc8d2' }}><option value="">Choose learner</option>{learners.map((user) => <option key={user.id} value={user.id}>{user.name} · {user.email}</option>)}</select></label><a href="/auth/admin/google/reauthenticate?return=/roles" className="mt-5 block rounded-xl border-2 border-[#88ceff] px-4 py-3 text-center font-bold text-[#006590]">Verify with Google</a><button className="mt-4 w-full rounded-xl px-5 py-3 font-black text-white" style={{ background: '#006590', borderBottom: '4px solid #004c6e' }}>Assign scope</button></form>
