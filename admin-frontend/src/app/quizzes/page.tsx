...
function PageContent() {
  ...
  return <div className="space-y-6"><PageHeading title="Quiz performance" description="Hiệu suất quiz tổng hợp theo khóa học trong 30 ngày gần nhất." /><section className="grid gap-4 sm:grid-cols-2"><StatCard label="Attempts" value={data.attempts} /><StatCard label="Courses with attempts" value={data.by_course.length} accent="#843ab4" /></section><DataPanel title="Course breakdown">{data.by_course.length === 0 ? <StateNotice state="empty" /> : <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr><th className="p-3">Course</th><th className="p-3">Attempts</th><th className="p-3">Average score</th></tr></thead><tbody>{data.by_course.map((row) => <tr key={row.course_id} className="border-t"><td className="p-3 font-bold">{row.course_title}</td><td className="p-3">{row.attempts ?? 0}</td><td className="p-3">{row.average_score == null ? '—' : `${row.average_score}%`}</td></tr>)}</tbody></table></div>}</DataPanel></div>;
}
