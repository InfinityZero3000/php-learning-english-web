...
export function CourseOutline({ course }: { course: AdminCourse }) {
  ...
  return <section className="mt-8" aria-labelledby="course-outline-heading">
    ...
    {editing && <form onSubmit={saveUnit} className="mt-5 grid gap-3 rounded-2xl border-2 border-[#006590] bg-[#e8f4ff] p-5 md:grid-cols-2">
      ...
      <div className="flex justify-end gap-3 md:col-span-2"><button type="button" onClick={() => setEditing(undefined)}>Cancel</button><button disabled={busy} className="rounded-xl bg-[#006590] px-4 py-2 font-bold text-white">Save unit</button></div>
    </form>}
      ...
      {units.map((unit, index) => <article key={unit.id} aria-label={unit.title} className="overflow-hidden rounded-3xl border-2 border-[#bdc8d2] border-b-4 bg-white">
        <div className="flex flex-wrap items-start justify-between gap-4 p-5" style={{ backgroundColor: unitColor(unit.background_color) }}>
          ...
          <div className="flex flex-wrap gap-2">
            <button aria-label="Move up" disabled={busy || index === 0} onClick={() => move(index, -1)} className="rounded-lg bg-white px-3 py-2 font-bold disabled:opacity-30">↑</button>
            <button aria-label="Move down" disabled={busy || index === units.length - 1} onClick={() => move(index, 1)} className="rounded-lg bg-white px-3 py-2 font-bold disabled:opacity-30">↓</button>
            <button disabled={busy} onClick={() => setEditing({ id: unit.id, data: { course_id: course.id, title: unit.title, description: unit.description, sort_order: unit.sort_order, icon_url: unit.icon_url, background_color: unit.background_color } })}>Edit</button>
            {unit.status === 'draft' && <button disabled={busy} onClick={() => void run(() => adminUnits.publish(unit.id))}>Publish</button>}
            {unit.status !== 'archived' && <button disabled={busy} onClick={() => void run(() => adminUnits.archive(unit.id))}>Archive</button>}
            {unit.status === 'draft' && !unit.lessons.length && <button disabled={busy} onClick={() => { if (confirm(`Delete unit “${unit.title}”?`)) void run(() => adminUnits.delete(unit.id)); }} className="text-[#ba1a1a]">Delete</button>}
          </div>
        </div>
