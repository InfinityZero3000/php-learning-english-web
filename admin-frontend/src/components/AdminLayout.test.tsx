...
vi.mock('@/lib/api', async (importOriginal) => {
  ...
  return { ...actual, auth: { me: vi.fn(), logout: vi.fn() } };
});
...
describe('AdminLayout guard', () => {
  ...
  it('renders for admins', async () => {
    vi.mocked(auth.me).mockResolvedValue({ id: 1, role: 'admin' } as never);
  ...
  it('logs out non-admin users', async () => {
    vi.mocked(auth.me).mockResolvedValue({ id: 2, role: 'learner' } as never);
