...
export function ResetPasswordPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try { await auth.resetPassword({ token, email, password, password_confirmation: confirmation }); router.replace("/login?reset=1"); } catch (cause) { setError(messageFor(cause, "Không thể đặt lại mật khẩu.")); } finally { setSubmitting(false); }
  }
