...
export function LoginPage() {
  ...
  return <AuthLayout title="Welcome back" description="Pick up where you left off and keep your English growing.">
    ...
    <form onSubmit={submit} className="space-y-4"><Field id="email" label="Email" type="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} required /><div><Field id="password" label="Password" type={visible ? "text" : "password"} autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required /><button type="button" onClick={() => setVisible(!visible)} className="mt-2 text-xs font-bold text-primary">{visible ? "Hide" : "Show"} password</button></div><div className="text-right"><AuthLink href="/forgot-password">Forgot password?</AuthLink></div><Button className="w-full" size="lg" disabled={pending}>{pending ? "Signing in..." : "Sign in"}</Button></form>
