...
- `routes/web.php`, `routes/api.php`: route public, admin placeholder và API trạng thái.
...
- Role có nhiều User.
- Level có nhiều Course và Course thuộc một Level; Topic có quan hệ nhiều-nhiều với Course và một-nhiều với Vocabulary.
...
Nếu máy thực thi thiếu PHP/Composer host nhưng có Docker, mọi kiểm tra chạy trong container. Nếu không có Docker CLI, chỉ rà soát tĩnh cấu hình Compose và route/schema. Nếu có CLI nhưng Docker daemon không chạy hoặc không thể tải image/package, chạy `docker compose config` nếu khả dụng rồi rà soát file; trong mọi trường hợp phải ghi rõ phần runtime chưa thực hiện và README vẫn cung cấp lệnh xác minh chuẩn cho nhóm.
